...
//  DatabaseExpense db = DatabaseExpense();
  ...
  double dailyExpenseSum(List expense) {
    var totalDailyExpenses = expense.map((e) => e['itemPrice']).toList();
    var totalDailyExpenseQuantity =
        expense.map((e) => e['itemQuantity']).toList();
    for (int xx = 0; xx < totalDailyExpenses.length; xx++) {
      totDailySum += (double.parse(totalDailyExpenses[xx]) *
          double.parse(totalDailyExpenseQuantity[xx]));
    }
    return totDailySum;
  }
  //
